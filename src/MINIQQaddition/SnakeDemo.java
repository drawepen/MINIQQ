...
public class SnakeDemo extends JComponent{
    ...
    private JLabel label3 = new JLabel("游戏规则:");
    ...
    public SnakeDemo(){
        ...
        p.setBorder(BorderFactory.createLineBorder(Color.black));         //设置边框
    ...
    public void paintComponent(Graphics g1){
        ...
        g.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL,RenderingHints.VALUE_STROKE_NORMALIZE);//笔划规范化
        
        //画头部
        g.setColor(Color.orange);
        g.fillRoundRect(head.x, head.y, 20, 20, 10, 10);
        
        g.setPaint(new GradientPaint(115,135,Color.CYAN,230,270,Color.MAGENTA,true));     //循环渐变
        if(!first_launch)
        {
            //初始化身体
            int x = head.x;
...
//                    }
        ...
        run = new Thread() {
            public void run() {
                while (true) 
                {
                    ...
                    if(head.x>0&&head.x<475&&head.y>0&&head.y<460)
                    {
	                     if(!pause)
	                     {    
	                         temp.x = head.x;
    ...
    public static void main(String[] args) {
        ...
        game.setIconImage((new ImageIcon("image\\snake.jpg")).getImage());   //设置头标题
